...
export function isFirstVip(userId) {
    return fetchs(CONFIG.DOMAIN_PREFIX.user + '/api/vip/isFirstVip', { userId: userId }, 'GET');
}
...
export function prizeList(userId) {
    return fetchs(CONFIG.DOMAIN_PREFIX.default + '/api/jsonp/pf?url=/prize/getPrizeList', { userId: userId }, 'GET');
}
 ...
 */
export function couponList(userId, couponType, offset = 1, pageNum = 20) {
    return fetchs(CONFIG.DOMAIN_PREFIX.default + '/api/jsonp/pf?url=/coupon/couponlist', { userId: userId, type: couponType, offset: offset, pageNum: pageNum }, 'GET');
}
 ...
 */
export function getOrderList(orderStatus, userId, pageSize = 1) {
    return fetchs(CONFIG.DOMAIN_PREFIX.default + `/api/uc/jsonp/order/${orderStatus}/${userId}/list`, { p: pageSize }, 'GET');
}
...
export function getOrderDetail(orderId, userId) {
    return fetchs(CONFIG.DOMAIN_PREFIX.default + '/api/jsonp/pf?url=/order/getOrderDetail', { userId: userId, orderId: orderId }, 'GET');
}
